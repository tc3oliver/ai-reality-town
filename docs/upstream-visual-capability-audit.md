...
| **Legacy Canon Test Fixture** (renamed by this task) | `convex/canon/mistwoodFixture.ts` → **`convex/canon/legacyCanonTestFixture.ts`** | Two characters (Cassia, Rowan) at `mistwood-market`/`mistwood-grove` — locations that do not exist in the production seed. Predates PRD 2.0; used only by three pre-existing, unrelated Canon foundation tests (`convex/canon/reducer.test.ts`, `convex/canon/replay.test.ts`, `convex/knowledge/canonCognitionIntegration.test.ts`) plus its own `mistwoodFixture.test.ts`. **Decision: renamed, not rebuilt.** Rebuilding it to reuse production seed IDs would risk perturbing three passing, unrelated PRD 1.0 foundation tests for no V2 benefit; renaming makes its non-production nature unambiguous at the call site while leaving that established test behavior untouched. |
| **V2 Visual Runtime Fixture** | None exists yet | No V2 task (ART-108–ART-140) has created a dedicated visual-runtime test fixture as of this audit. Any future one must derive its character/location IDs from `mistwoodSeed.ts`, never from `legacyCanonTestFixture.ts`. |

**Binding rule (must be followed by every downstream task):** V2 Dynamic Live production
acceptance — **ART-119 production acceptance, ART-137, ART-138** — must never use the
Cassia/Rowan legacy fixture. Those tasks' descriptions have been updated to reference
`legacyCanonTestFixture.ts` by its new name and restate this rule (see backlog task edits
