...
- Contract, validation and zone arrival: `convex/visualBinding/locationBinding.ts`
- The eight Mistwood bindings: `convex/visualBinding/mistwoodLocationBindings.ts`
...
`createdAt`/`updatedAt` from PRD §14.2 belong to persistence and arrive with the runtime snapshot
work; the authored set here is a versioned constant, pinned by `MISTWOOD_LOCATION_BINDING_VERSION`.
