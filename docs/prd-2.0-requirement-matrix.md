...
| FR-O004 公開交談與活動提示 | New | ART-123 | **Done** | P0 | **這不是新增 schema,而是點亮一個自 ART-115 起就休眠的契約**:`PublicAnimationState` 早已宣告 `speaking｜thinking｜activity`,validators 驗它,`CharacterStateIndicator` **已經會畫**對話泡泡與思考雲(`characterAnimation.ts` 的 `SPEECH_PRIMITIVES`),但從來沒有任何伺服端程式碼產生過這三個值——該檔與 `docs/character-motion-rendering.md` 都明寫 FR-O004 是會產生它們的那個任務,故 Schema Impact 為 None 是對的。**決定一:畫布上不放任何世界衍生的文字**。AC#1 允許「摘要、狀態**或**短泡泡」,選擇既有向量指示器而非 `PIXI.Text` 字幕,使 AC#2／AC#3 在地圖表面成為**結構性保證而非紀律**:投影文字根本到不了 renderer。**此處據實修正了本任務自己的第一版主張**:畫布**並非**完全無文字——`MapZoneLayer` 會畫八個**作者撰寫**的地點名稱(`data/mistwood.ts` 的 repository 常數,每次部署都相同,依構造即為公開),那不是洩漏面。真正該主張且準確的是更窄的那一句:**畫布上沒有任何文字來自投影 payload**;`conversationHints.test.ts` 釘住 `MapZoneLayer` 是 renderer 中唯一的文字建構子、其來源是 `footprint.name`、且沒有任何 renderer 檔案讀取 `summary`／`publicSummary`／`dialogue`／`summaryText`。公開摘要留在 DOM(場景面板與角色卡),那裡本來就已經過 withhold 替換、可被螢幕閱讀器讀出、可被選取。**決定二:狀態源於參與,文字源於發佈**。角色因為是 `active` 場景的參與者而讀作 `speaking`,且該判定**完全不參照 `publicationStatus`**——這直接就是 AC#5(沒有可發佈文字時仍顯示安全狀態),也維持了 FR-P004 已有的保證:ART-132 AC#4 要求 withhold 文字不得移動任何角色,同理也不得改變其動畫狀態,否則 withhold 會在地圖上以行為變化的形式被看見;而 withheld 場景的存在、地點與參與者本來就仍公開(`activeScenePresentation` 只清空 `title`／`summary`),故不構成新的洩漏面。參與者 >=2 → `speaking`,==1 → `activity`,已結束場景不貢獻任何狀態(對一小時前談完的人畫泡泡是關於「現在」的宣稱);**`thinking` 永不產生**——Canon 沒有記錄任何角色正在思考,從參與人數推出內在狀態正是地圖存在以避免的 RISK2-008 違反,該值維持宣告與可算繪,留給日後真的有東西記錄它時。**僅在 motion 為 `idle` 時套用**:走路中的角色是在路上,在行走人形上畫泡泡等於宣稱他站著在講話,已發布的 motion 是更具體的事實並保有優先權。**決定三:不新增投影欄位**。短提示是 `summary` 的純函式,而 `summary` 已發佈且已經過 withhold 替換;新增第二個「另外截斷的同一段已發布文字」欄位等於製造**第二個必須套用 withhold 替換的地方**,即第二個洩漏面,卻不帶來任何新資訊。改由客戶端從已替換的 summary 推導:withheld 場景的 summary 是 `''`,故提示**依構造**為空,沒有第二條可能寫錯的程式路徑。`truncateForPublic` 放在 `convex/shared` 使兩端截斷方式相同(兩套規則會讓卡片與面板對「句子在哪裡結束」有不同答案),且省略號**計入**預算內(先截斷再附加會比呼叫端所設的欄寬多出一個字元,那正是提示在唯一沒人測過的螢幕尺寸上把卡片撐寬的方式);刻意不做斷詞,因為中文沒有詞界,而只對空格分隔的那一半有效的規則仍會把 CJK 切在任意位置,卻在覆核時看起來正確。**卡片新增兩列**:「與 X、Y 交談中」(以 id,因投影不發布顯示名,與鏡頭控制及場景面板一致)、以及「談話內容:…」;後者在文字被 withhold 或未發佈時**是缺席而非空白**——空白列會暗示這場對話沒有內容,而上方的狀態列仍說著他們正在交談。兩個場景的摘要**永不併接**(併接會產生一句兩個場景都沒發布過的話,正是 FR-P004 provenance 規則所禁止的衍生文字)。場景面板首次讀取 `publicationStatus` 並標示「(內容審核中)」——伺服器自 FR-P004 起就已替換佔位字串故未曾洩漏,但面板把該佔位字串當成場景的實際標題呈現,沒有任何東西說明內容被保留;地點與參與者保留,因為地圖上本來就畫著站在那裡的角色,場景從他們腳下消失是更大的謊。`PUBLIC_DYNAMIC_RUNTIME_VERSION` 3→4:沒有欄位改變形狀,但 `animationState` **重新詮釋**了兩個它一直宣告著的值,而 `docs/public-dynamic-projection.md` 要求既有欄位的重新詮釋也需 bump,不只形狀變更。**三個窮舉式 guard 攔下本工作,三次都正確**:replay builder 的相依閉包 pin、客戶端 bundle 的閉包 pin(含 type-only 邊)、以及場景面板的完整顯示紀錄 pin;`conversationState.ts` 除了一個 type 之外不 import 任何東西,故不可能是通往 Canon seed 的路徑——但那些 pin 的意義正是「把這句話變成一次經過覆核的決定而非假設」。**驗證**:`npm run check` 全綠(155 套件、2400+ 通過、5 個既有 skip、build 成功);`npm run e2e` 全綠(66 tests)。三次 fault injection(讓走路中的角色也拿到泡泡→1 失敗;省略號改為附加在預算外→4 失敗;提示不再檢查場景是否 active→1 失敗)反證斷言非空過。見 `docs/public-conversation-hints.md` | ART-119 |
...
| FR-O010 動態畫面降級 | New | ART-127 | **Done** | P0 | 四級中兩級早已存在、**一級已建好卻從未接線**、一級不存在,故本任務不是加一個降級開關,而是**把懸崖改成階梯**。`stream`(ART-118／119)與 `informational`(`LiveMapFallback`)早已有;`snapshot` 自 ART-116 起就發布 `characterStates`／`activeSceneStates` 與伺服器裁定的 `freshness`,而**實況地圖從來沒讀過它**——唯一的讀者是首頁,而且只取 freshness 畫徽章;`static-map` 完全不存在。原本的行為是:`webglSupported` 一為 false 就整頁跳到文字實況指路頁,中間兩級全部略過。**AC#5 之所以不需要任何復原機制,是因為等級是純推導而非閂鎖**:`resolveDegradationLevel` 是每次算繪都會重算的純函式,投影一回來,下一次算繪就已經是 `stream`——沒有 polling、沒有計時器、沒有重試。若改以 `useState('degraded')` 閂鎖,就得另造一套「何時該試著爬回去」的機制,而那套機制本身正是 AC#4 要禁止的重試迴圈。唯一必須閂鎖的是**擲出的算繪器**(重掛剛崩潰的算繪器即 crash loop,而對線上部署的 crash loop 正是把算繪故障放大成整個後端負載的形狀),但它閂的是**算繪器**而非等級:其下各級仍隨資料自動升降,閂鎖僅在 `mapId` 改變時清除,絕不依時間。**錯誤邊界因此往內搬**:`LiveMapErrorBoundary` 包住整條路由,算繪器一擲出就連同頁面上四個公開讀取一起卸載——那在「唯一的答案是換一頁」時是對的,卻讓中間兩級**結構上不可達**,因為第 3 級要的正是**資料活過算繪器**。故新增只包住 Pixi stage 的 `RendererErrorBoundary`,擲出只帶走 canvas:query 仍訂閱著、view model 仍組好、頁面下一次算繪就換上平面圖;`LiveMapErrorBoundary` 留在原處作為頁面本身失敗(讀取擲出、view model 建不起來)的最外層網,那種情況才真的無級可降。**第 3 級是 DOM/SVG 而非「凍住的 Pixi」**:該級存在的前提就是 Pixi 跑不動,凍結的 canvas 在唯一需要它的情境下根本不可達。以 `mistwoodLocationFootprints` 畫 SVG 平面圖另有兩個好處:它是真實 DOM 故螢幕閱讀器讀得到(動態地圖是 `<canvas>`,對輔助技術天生不透明,這正是 ART-113 把所有控制項放在 canvas 旁 DOM 的原因;這裡連**內容**都在 DOM),且保留了純文字清單會丟掉的空間資訊——第 3 級存在的意義就是要比第 4 級多。**位置只有一個來源**:平面圖由 Pixi 消費的**同一份** `ReadOnlyWorldViewModel` 與鏡頭控制建構所用的**同一份** `FocusTarget[]` 投影而來,不重新推導任何座標、不重新查任何名字,故靜態圖與動態圖**結構上不可能**對「誰在哪」有不同說法——那正是第二條算繪路徑最典型也最糟的失效模式(只在沒人看的降級狀態下才現形)。連帶的後果明說:`focusTargetsFrom` 今天以 id 標示角色(動態投影不發布顯示名),平面圖因此也是;這份**一致**才是值得釘住的性質。**舞台在每一級都維持原形**:`.live-map-canvas` 恆為 `.live-stage` 恰兩個子元素中的第一個,ART-126 證明的響應式契約(<64rem 一欄地圖在前、>=64rem 兩欄、兩者視覺次序皆等於 DOM 次序)不因降級而改變,只有那個盒子**裡面**的東西變;`data-rung` 讓樣式表放寬固定高度(canvas 的 clamp 對會隨容器縮放的東西是對的,對有內在高度的平面圖加名冊則會裁掉——而在專職回答「大家在哪」的那一級裁掉那份清單,等於廢掉該級)。**AC#4 由三個方向封閉**:(a) `liveMapSurface.test.ts` 掃描本模組每個出貨檔案的 write／request API,新檔案自動納入,且它**逐一具名列舉**讀取——因此接上第 2 級的 query 是一次紅燈、需要刻意且經覆核的編輯,而非默默多出第七個訂閱;(b) **query 集合不得依賴等級**:天真實作會在降級時重新訂閱(`key`、條件式讀取、隨等級翻轉的 `'skip'`),任一種都會把算繪故障變成對一個已經在出狀況的部署的 query 洪水——測試以**平衡括號**而非 regex 從 `LiveMapPage.tsx` 抽出每一個 `useQuery(...)` 並要求全都不提及 ladder(貪婪 regex 會吞掉整個檔案而讓斷言對一切都成立,非貪婪則停在第一個 `)` 而錯過真正該看的參數,兩者都試過);(c) 掛載後的失敗路徑實測不呼叫 `fetch`／`setTimeout`／`setInterval`,瀏覽器套件並在拒絕 WebGL 後監看網路三秒——足夠讓重試迴圈現形。**AC#2 以可觀察的強形式證明**:瀏覽器套件在拒絕 WebGL 與正常兩種情況下各載入一次 Episode 頁,要求算繪出的文字**逐字相同**。**故障注入在真實瀏覽器、且不留任何 production 測試鉤子**:jsdom 同樣沒有 WebGL,故一切「WebGL 不在時會怎樣」的 DOM 層斷言都是在一個 WebGL 從未存在過的環境裡做的——只有真實引擎能先給它 GPU 再拿走。`page.addInitScript` 在任何應用程式碼執行前覆寫 `HTMLCanvasElement.prototype.getContext`,對 `webgl`／`webgl2` 回傳 null 而其餘照舊(刻意保留 2D:全部拒絕會連 sprite 解碼一起壞掉,把「沒有 WebGL」變成「完全沒有 canvas」這個不同且罕見得多的故障);**沒有 `?degrade=` 參數、出貨 bundle 內沒有旗標**——query string 開關比較好寫,而且測到的是那個開關。**瀏覽器證據當場抓到兩個缺陷**:(1) 平面圖畫的是**重播**的 motions,於是只顯示當前重播場景裡的那一位而非全部十二位——FR-O013 的重播是**視覺**重播,靜態平面圖不會動,「在平面圖上重播」只會讓圖無聲地在數小時前的歷史位置之間跳、卻自稱「最後已知位置」;修法是在不算繪動畫的兩級直接**收回 replay 本身**(而非在四個地方分支),如此自動播放的 effect 提早返回、`advanceReplay` 無可推進、`replayFrame` 為 null、控制項回報無可重播,且**無須復原**——算繪器一恢復 replay 就自己回來,與階梯其餘部分同一套 derive-don't-latch 性質。(2) E2E fixture 的 `updatedAt` 是 `1_000` 這個哨兵值,在沒有東西把它當時間讀之前都不算錯,但階梯一開始從它算相對年齡,地圖就理直氣壯地宣告「20689 天前更新」;另外 `fixtureRuntimeSnapshot().characterStates` 是 `[]`——一個**空的**最後有效快照不是可信的 fixture(該表存在的全部理由就是在投影失效時持有世界狀態),之所以沒人發現,正是因為在第 2 級接上之前,實況地圖根本沒讀過它。兩者皆已修正並由 `fixtureWorld.test.ts` 釘住。**未涵蓋**:第 2 級沒有瀏覽器證據(要在真實瀏覽器裡讓投影消失需要一個 fixture 開關,而該級已由單元與 DOM 測試完整覆蓋,瀏覽器獨有的價值在第 3 級的真實 WebGL 拒絕);連線中途遺失 GPU context(邊界對任何來源的擲出都成立,套件注入的是「從來就沒有」那一種,模擬中途遺失需要 headless profile 未提供的 WebGL 擴充);第 4 級的「最後更新」標籤不會跳動(`useMotionClock` 在無可動畫時停駐,而階梯刻意不加第二個計時器——`liveMapSurface.test.ts` 斷言本模組不掛任何重複計時器——故年齡在掛載時正確、其後凍結,而那正是本來就沒有東西在動的一級;第 1–3 級都有 motions,計時器照跑);第 2 級沒有世界時鐘(runtime snapshot 帶位置與場景但不帶 `worldDay`／`timeSlot`,故該級無日夜色調與時鐘 chip,組合器本就容忍,把欄位加進快照契約是本任務不需要的 schema 變更)。見 `docs/dynamic-view-degradation.md` | ART-116, ART-118 |
...
| Requirement | Disposition | Task | Delivery State | Release Criticality | 重用證據 | Dependencies |
|---|---|---|---|---|---|---|
| FR-P001 動態首頁入口 | New | ART-129 | **Done** | P0 | 第一畫面是**一個** `<section class="home-first-screen">`,不是五個。五個會在設計系統的 `main > section` 規則下變成五個 `<h2>` 加五張卡——那正是一份目錄,而目錄正是 UX2-001 要求停止的東西;一個 section 才是一個螢幕。內含 AC#2 全部欄位:實況地圖為首要動作(AC#1,旁邊恆常並列文字實況等價入口,NFR-009 AC#3)、至多四位居民**畫出來**、現況摘要、主線 arc、最新大事、至多兩個進行中場景、推薦 Episode。**零新增查詢**:`activeArcs`／`activeScenes` 是首頁**早已**為其實況區塊抓取的 `live:<worldId>` 同一個已發布模型的另外兩個欄位,故 Security Impact(公開首頁不得觸發 LLM)依構造不變——沒有讀任何新東西,而讀的都是已發布投影。兩者皆為 optional 且皆以 `Array.isArray` 而非 `?? []` 檢查:payload 是無型別的已發布模型,畸形的那一份必須讓第一畫面降級,而不是讓首頁空白。**畫居民(AC#3／AC#4)不重用地圖的頭像**:`CharacterCard` 經 `useSpriteAssets()` 在 `<canvas>` 上重新著色調色盤變體再回傳 data URL,那套機器在 `clientLive`,而 `clientPublic` 不得依賴 `clientLive`(反向邊已存在,import 即循環);抽成第三個模組是另一個選項,但為了在首頁放十二張 32×32 的圖而重構地圖的資產管線並不划算。`data/` 不屬於任何邊界模組,故 `CharacterSprite` 直接讀 `data/spritesheets/catalogue` 的 `CHARACTER_TEXTURE_URL`／`SPRITE_CELL_ORIGINS`／`SPRITE_FRAME_SIZE` 以 `background-position` 切出正面影格——**完全無須任何邊界變更**。**唯一的差異明說而非隱藏**:首頁畫的是 **base** cell 而非重新著色的變體,霧林鎮十二位居民中有四位是他人 sprite 的調色盤變體(`pei-lan`/`f1`、`wu-zhen`/`f2`、`fang-yue`/`f4`、`zhao-ming`/`f6`),故這樣的一對在首頁會以 base 調色盤呈現同一個人形;但他們的**綁定**仍是自己的(sprite key 正是 FR-N004 指派的身分),名字也以真實文字寫在旁邊。**sprite key 必須取綁定自己的 key 而非資產 key**:`mistwoodCharacterSpriteKeys` 映到的是**資產** key,調色盤變體會帶後綴(`f6:mistwood-indigo-hair`),那根本不是 sprite key、`isSpriteKey` 會拒絕——本任務第一版正是因此讓四位居民渲染成 `data-sprite="none"`;現改走 `MISTWOOD_CHARACTER_VISUALS.find(...)?.spriteKey`,並由測試釘住每位霧林鎮居民都解析到 `isSpriteKey` 接受的 key。無綁定者渲染 `data-sprite="none"` 而非借用他人外觀(FR-N004 AC#6)。sprite 為 `aria-hidden="true"`:名字就在旁邊且是真實文字,再宣告一次等於同一則資訊念兩遍。**主線 arc 的排序表是 restate 而非 import**(同一個邊界理由),但漂移會比重複更糟——首頁與實況 overlay 會指認**不同**的 arc 為「那條主線」,比兩者都不指認更糟——故 `homeRoute.test.ts` 讀取 `storyOverlayModel.ts` 的**原始碼文字**並斷言兩張表一致,改一邊不改另一邊是紅燈而非靜默分歧。`pickPrimaryArc` 為全域函式且對**未知** status 排最後但仍保留資格,故日後新增的生命週期階段降級為「排在最後」而非「從首頁消失」。AC#5 的連結:居民→`#character/`、arc→`#arc/`、場景與推薦→`#episode/`(場景的頁面**就是**當日 Episode,那是 FR-P002 的連續性規則而非捷徑);整塊 tile 是 `<a>` 而非只有名字,因為 64px sprite 旁的 32px 名字在手機上是會按空的目標,而 sprite 才是眼睛去的地方。**瀏覽器證據**:`e2e/dynamicView.spec.ts` 新增首頁 describe,AC#3／#4 以 `new Image()` 實際載入紋理——指向 404 的 `background-image` 在 DOM 中與正確的一模一樣,只斷言 URL 等於什麼都沒斷言;AC#6 同時讀 `window.__ART137__` 與瀏覽器自身網路層。另修一個 fixture 缺陷:onboarding fixture 原本沒有 `characters`,第一畫面因此無人可畫,現已補齊並由 `fixtureWorld.test.ts` 釘住。**驗證**:`npm run check` 全綠、`npm run e2e` desktop 與 mobile 兩個 project 全綠。**未以機器驗證者**:第一畫面對人是否真的讀起來像「一個活著的世界」——那是 UX2-001 的美學那一半,比照 FR-P003 記為人工覆核並歸於 ART-138 發布關卡。舊的 認識這個世界 區塊中有兩樣東西已移除,理由同一個——AC#2 把兩者都放上第一畫面,留著原件等於同一件事在一頁講兩次:文字版角色清單(同四個名字兩次),以及 推薦入坑點(指向**同一集**的第二個連結,對依連結導覽的人就是同一個目的地兩次)。其「尚未推薦」空狀態隨之搬上第一畫面,故觀看者仍分得出「沒有推薦」與「這一段壞了」;測試以**整頁**計數斷言該 href 恰好一個且位於第一畫面內,並以注入重複連結證明該斷言非空轉。見 `docs/homepage-composition.md` | ART-118, ART-111 |
...
| FR-Q002 管理者動態觀看控制 | New | ART-134 | **Done** | P1 | 五個控制項(暫停公開更新、釘住最後有效快照、隱藏單一角色／場景視覺、重建投影)＋一個唯讀檢視,全部經 `opsConsoleFunctions` 的**同一道** `requireOperator` 與 `recordAudit`,而非另立第二套機制——該檔案自己的註解本來就要求如此,FR-K006 的緊急停止控制也是這個先例。連帶繼承的是**整道閘門**而非只有檢查:registry 未設定時 fail closed、identity 優先於 token 與 `CLERK_JWT_ISSUER_DOMAIN` 切換規則、以及 audit row 寫在命令自己的交易內——故擲出的命令不會留下宣稱已套用的 audit row,而 audit row 建不起來的命令也不會套用。**Ledger 為 append-only 且效果由重播推導**(與 FR-P004 的 `safetyStatusOverrides` 同一形狀):**解除是一列,不是刪除**。理由不是整潔——操作者控制決定公眾能看到什麼,事後要問的從來不只是「現在藏著嗎」,而是「誰藏的、何時、為什麼、又解除了什麼」;可變的 `hidden: boolean` 只答得出第一個並摧毀其餘。**與安全 withhold 刻意分離**:那張表記錄**分類器**拒絕的內容,這張記錄**操作者**拉下的內容;合併會讓這裡的解除靜默地 un-withhold 掉安全拒絕的東西,也會讓兩者在稽核中無法分辨——`dynamic.hide` 因此不併入 `safety.override`,`dynamic.pause` 也不併入 `world.pause`(暫停**世界**是停模擬,暫停動態**檢視**是在世界繼續跑的同時停止republish 公開畫面,共用一個 capability 等於讓能凍結公開頁的人也能停下 Canon)。**在 build time 套用而非 read time**:read-time filter 會把被隱藏的角色留在已儲存 payload 裡,而 FR-O010 的 last-known-good 會在當前版本讀不到的那一刻繼續服務它——被隱藏的東西會從「用來讓頁面活著」的那個機制裡回來;FR-P004 的安全閘門正是為此在 build time 套用。**被隱藏的角色是移除而非留白**(與 FR-P004 對 withheld 場景的選擇相反,且刻意如此:場景留白是因為地圖的職責是呈現世界所在,角色明明站著而場景消失是更大的謊;但人的位置沒有誠實的佔位符,畫一個「這裡有人但我們不說是誰」的標記比隱藏透露更多),並同時從每個場景的 `participantCharacterIds` 移除——被隱藏的**就是那個 id**,留著等於在場景面板上繼續公布「此人在該場景」。**純模型放在 `convex/shared`**:兩個模組必須對它有共識,`operations` 擁有命令而 `publicRead` 必須在建投影時套用結果,但邊界禁止 `publicRead` 依賴 `operations`(該邊已朝反方向存在);另一個選項是讓投影建構器自行重新推導,那是「什麼被隱藏」的兩份實作,而它們分歧的方式正是:操作者藏起來的東西留在畫面上,而主控台仍回報它已被隱藏。**AC#4 已由 ART-133 交付,本任務刻意不重做**:`inspectDynamicViewMetrics` 早已以 `server_measured` 導出 `missingCharacterBinding`／`missingLocationBinding`／`canonRuntimeLocationMismatch` 且同樣經操作者閘門;從控制主控台再呼叫並轉發它的輸出,等於給主控台兩個讀同一組數字的地方,而那的結局是其中一個對同一個世界回報不同的數字。**AC#6／#7 以結構證明**:純模型**完全沒有 import**(故不可能觸及 Canon、修正函式或資料庫,且不可能在本測試不紅的情況下取得一個);wiring 不提及任何 canon 路徑,也不提及 `compensate`／`retcon`／`rollback`;唯一寫入的表是自己的 ledger(以正規表示式自原始碼抽出而非目測);且從不 patch／replace／delete。重建是唯一碰到投影管線的命令,而它是滿足 AC#5 的最小權力形式:讀取**已然如此**的 Canon,再寫入 read-model store——與 post-commit orchestrator 每次接受事件後所做的完全相同,`rebuildLiveProjection` 維持 `internalMutation`(把它改為可公開呼叫來滿足 AC#5,等於在公開表面上放一個未經認證的 rebuild)。**暫停期間的重建是拒絕而非默默執行**:暫停是操作者對「公開畫面應停止移動」的陳述,執行重建會移動它,而默默做比拒絕更糟;解除只差一次呼叫且會出現在稽核軌跡上,那正是「推翻另一個決定」該留下紀錄的地方。**兩道既有閘門攔下本分支,兩次都是它們在正確運作**:`operatorAuthorization.test.ts` 逐一列舉 capability,五個新的必須刻意加入;`publicReadOnlyGuarantee.test.ts` 原本禁止**任何**公開函式宣告 `characterId` 引數(那是玩家控制 API 的形狀),而 `setCharacterVisualHidden` 正當地需要它——現改為對 **anonymous** 函式維持絕對禁止,並**新增**一條「凡宣告 `characterId` 者必須是 operator-gated」,這是**收窄**而非放寬:另一個選項是具名豁免單一函式,而具名豁免正是窮舉式閘門停止窮舉的方式。**驗證**:`npm run check` 全綠(156 套件、2437 通過、5 個既有 skip、build 成功);`npm run e2e` 全綠(66 tests)。三次 fault injection,而**其中兩次一開始沒被抓到,這本身是本任務的發現**:把 `recordAudit` 包進 `if (false)` 通過了整個結構性套件(符號還在),把 `applyDynamicViewControls(...)` 換成 `derived.projection` 也通過了(import 行還在)。前者的修法是新增**行為式**套件,以真實 registered handler 對假 ctx 執行並斷言實際寫入了什麼;後者的修法是在檢查前把所有 import 敘述剝除。兩個缺陷都不在產品程式碼裡,而在**證據**裡,而一份看起來在證明某件事卻沒有的證據,比沒有證據更糟。見 `docs/dynamic-view-operator-controls.md` | ART-133 |
| FR-Q003 Incremental Public Projection | **Carry Forward** | **ART-100**（既有） | To Do | P1 | — | ART-115 |
| FR-Q004 動態視圖無障礙基線 | New | ART-135 | **Done** | P0 | 以 ART-137 的 Playwright 套件在 desktop 與 mobile 兩個 project 對 WCAG 2.1 A/AA tag 斷言(每 project 15 測試,共 30 全綠),刻意**不**重述 `liveMap.a11y.test.tsx` 與 `publicPages.a11y.test.tsx` 已涵蓋的部分。理由:實況地圖是唯一無法自 markup 定案無障礙性的 P0 表面——它的核心是 `<canvas>`,而真正要緊的每一個主張都是關於瀏覽器拿到 canvas 之後會怎樣:Tab 是否真的走得到控制項且焦點環真的被畫出來、Reduced Motion 是否真的停下動畫、canvas 對輔助技術是否真的惰性、axe 在實際算繪的頁面上是否通過。jsdom 沒有排版、沒有焦點環、沒有動畫,只證得了 markup。AC#1:自地圖的指標連結實際點進 `/live/<worldId>/text`,該頁**無 canvas**、以文字承載角色／地點／場景、且 axe 乾淨。AC#2:控制項**以按鍵**抵達、以 **Enter** 啟動,焦點移入卡片、關閉後回到觸發控制項;連按 25 次 Tab 持續前進不成陷阱;canvas 無 `tabindex`／`role`／handler;焦點環以 `outlineStyle !== none` 且 ≥2px **實際被畫出來**量測——`:focus-visible` 是關於「焦點如何抵達」的啟發式,故必須以按鍵而非 `locator.focus()` 抵達,否則量到的是 `outline: none` 並回報一個並不存在的缺陷。AC#3:模擬該偏好後,重播不自動播放、手動控制仍可用、`<main>` 底下無任何元素的 transition／animation ≥1ms(數值解析而非字串比對:守衛宣告 `0.01ms`,各引擎序列化不一致為 `0.00001s`／`1e-05s`,字串過濾會在某個引擎上回報 276 個假違規)。AC#4:播放期間三個時間狀態同時在畫面上,且**文字互異、computed `border-left-style` 互異、`aria-hidden` glyph 互異**——自引擎讀取,因為那是 `data-state` 選擇器真正變成邊框的唯一地方。AC#5:四個帶不同 `animationState` 的角色產生四個互異的卡片讀數(以文字),overlay 的世界時鐘在收合後仍存在。**Reduced Motion 抑制什麼、不抑制什麼**:ambient 漂移與環境動畫由 seed 而非 Canon 推導,予以抑制;**Canon 驅動的移動不抑制**,且這是刻意的——角色是真的移動了,藏起來等於謊報世界。重播的**自動播放**被抑制,重播本身仍可依請求取得,故無任何東西變得不可達。因此本套件**不**斷言「canvas 停止變化」:早期版本這樣寫並且失敗了,而且失敗得正確,因為 fixture 有一個角色正在行走;像素比對分辨不出 ambient 漂移與 Canon 移動,故抑制在能被隔離的地方證明——`ambientMotion.test.ts` 與 `environmentAnimation.dom.test.tsx`,決定性地。**撰寫過程的兩個發現**:(1) 文字實況頁在 E2E fixture 下**完全沒有算繪**,因為 fixture 的 `live:<worldId>` payload 只帶了故事 overlay 會讀的 arcs;地圖套件永遠抓不到這個——地圖根本不讀 `characters`／`locations`／`recentEvents`。非地圖等價視圖是發布關卡,而實際去操作它的是無障礙套件。(2) `test.use({ reducedMotion: 'reduce' })` **沒有生效到頁面上**,整個群組其實在對普通頁面做斷言並以錯誤的理由通過;之所以被抓到,只因為每個測試在據此下任何結論之前都先檢查「模擬是真的」——這個檢查因此保留。**未涵蓋且不受本任務影響**:關係圖與時間軸表面(ART-94,依 PRD 2.0 §13 順延),以及 `docs/accessibility.md` §4 的人工檢查。見 `docs/accessibility.md` §6 | ART-126, ART-120 |
| FR-Q005 效能基準與品質分級（實現 NFR2-002） | New | ART-136 | **Harness 已交付;AC#4 行動版實測未達標且無法在本主機定案** | P0 | Harness 已交付並可重跑:`npm run bench`(build:e2e → Playwright → 報表),結果寫入 `docs/benchmarks/dynamic-view-latest.json`(機器可讀,為紀錄本身)與同名 `.md`(渲染,供覆核)。**讓它成為基準而非一組計時的是三件事,且三者都在輸出裡**:具名裝置檔＋固定 CPU 節流倍率(`desktop-reference` 1440×900 不節流;`mid-tier-mobile` Pixel 5 視埠與 DSF ＋ 4× 節流,該倍率取自 Chrome 自己的 Mid-tier mobile 預設,是業界參照點而非自創數字)、瀏覽器版本**與 GL renderer**、以及固定 zoom。renderer 由**頁面內讀取**而非依啟動旗標推斷:`--ignore-gpu-blocklist` 只是請求,沒有可用 GPU 的主機仍會靜默退回 SwiftShader,而硬體與軟體光柵化對 FPS 的差別幾乎就是全部。**四個模式全覆蓋(AC#10)**:`degraded` 以覆寫 `getContext` 產生,不需任何 production 鉤子;`delayed`／`snapshot` 是關於**伺服器回了什麼**的事實,瀏覽器端無論如何造不出來,故取自 `src/e2e/` 內的 scenario knob——production 只透過一個 import、一個 build-time 字面值到達該模組,`fixtureScenario.test.ts` 釘住「無任何出貨檔案 import 它」且它永不成為第二道 gate。`snapshot` **在 ART-127 之前根本無法量測**:實況地圖當時沒有讀取 runtime snapshot 的程式路徑。**三個 FPS 數字而非一個**:平均、P5、最差單幀。120 幀中一次 400ms 卡頓是最差的 0.8%,P5 會正確地回報 60——那是 P5 正確運作,也正是只報平均與百分位會放走一次可見凍結的原因。**記憶體以「回收後底線」量測,不是對原始樣本做迴歸**:第一版就是後者,而那錯得很像對的——每個 JS heap 都鋸齒,對鋸齒做最小平方除非週期恰好對齊取樣窗否則不會回 0,一個在 40→70 MiB 之間健康循環的頁面被報成將近 **3 MiB/min 的「成長」**,足以讓一個毫無洩漏的頁面 AC#7 紅燈;把門檻調高到不再誤報等於把閘門套在雜訊上。真正區分洩漏的是 heap **降不回原本的低點**,故改為切成六個時間窗、各取最小值(回收後底線)再看趨勢,`measure.test.ts` 雙向釘住:等幅鋸齒讀作 0,同一鋸齒但底線上升則被抓到。「末減初」錯在同一個根源且雙向皆錯。**AC#5 在真實引擎上補上單元測試證不到的那一半**:`motionQualityTiers.test.ts` 證的是純函式,本任務在 1× 與 6× CPU 下經**角色卡**讀同四位角色的地點並要求逐字相同——經卡片而非內部欄位,因為值得主張的是產品說了什麼。**AC#11 已達成**:結果已提交。**明列而非默默略過的缺口**(閘門若默默丟掉量不到的項目,就會為沒人量過的系統回報綠燈,那比回報缺口更糟,因為它看起來像證據):AC#2(query P95)與 AC#3(runtime→screen)是**傳輸與伺服器**性質,E2E build 的 transport 是同步 in-process fixture,在此量測只會記到約 0ms 並為一條從未被執行的路徑印出通過,標為 `requires_deployment` 並指向 ART-138(AC#3 的伺服器半邊 ART-133 已以 `runtimeProjectionLatency` 發布);**AC#6 的 20 與 40 位可見角色曾被標為 `unreachable`,那是錯的,ART-173 已修正**——原本的理由是:霧林鎮恰有十二位居民,`mistwoodCharacterSpriteKeys` 由 `MISTWOOD_CHARACTER_VISUALS` 產生、名冊被 `mistwoodCharacters.test.ts` 釘死在 `buildMistwoodCharacterVisualBindings()` 上,而 `composeReadOnlyWorldViewModel` 依 FR-N004 AC#6 **丟棄**無綁定角色。那些句子對**世界**全都成立,對**量測**則全不成立:NFR2-002 AC#4 問的是「動態層能否在 N 個動畫 sprite 下維持幀率」,不是主張鎮上有四十位居民,把它記成 unreachable 等於回報一個需求並未授予的限制。現以**合成負載探針**量測(`src/e2e/spriteLoadWorld.ts`、`src/e2e/benchEntry.tsx`):`bench.html` 是**第二個 Vite input**,`vite.config.ts` 僅在 `VITE_E2E_FIXTURE === '1'` 時加入,而只有 `build:e2e` 會設它;它直接掛載 `ReadOnlyWorld`,以 N 個合成行走角色輪替映射到**十二張真實** sprite sheet,因此不新增任何 `MISTWOOD_CHARACTER_VISUALS` 綁定、不動名冊釘死測試、也不讓任何 production 模組取得 benchmark 分支(`clientBenchProbe` 是獨立模組,且政策中**沒有任何模組**可依賴它,而 `clientProvider` 可依賴 `clientE2EFixture`——這正是不能直接放寬 fixture 的原因)。三項刻意的界線都寫在報表裡:樣本標為 `load probe`、**不發布 time-to-interactive**(裸掛載沒有頁面殼層,AC#1 仍屬實況頁),以及**僅 `stream` 模式**(`delayed`／`snapshot` 是關於伺服器回了什麼的事實,探針沒有伺服器;`degraded` rung 下 `LiveMapView` 根本不掛載 `ReadOnlyWorld`,「四十個 sprite 的 degraded」會是一個關於零個 sprite 的數字),兩項排除皆以 `not_applicable` 列記錄而非默默略過。桌機檔在 20 與 40 皆通過 AC#4 門檻;行動檔在同一軟體光柵化主機上兩者皆未達標,與下述行動版數字同因,記為 `measured_but_inconclusive`——**探針在本主機失敗是「20 與 40 現在可量測」的證據,不是關於手機的證據**。仍為 `unreachable` 的是 **AC#9 的另一半**:在該些角色數下固定地圖 zoom——探針未掛載擁有 zoom 的相機控制器。**AC#4 行動版實測未達標,且據實回報為 FAIL 而非豁免**:平均約 28 fps 對門檻 30。脈絡(不構成豁免):本主機無可用 GPU,即使忽略 GPU blocklist,Chromium 仍回報 ANGLE/SwiftShader,故行動檔在 4× CPU 節流下**軟體光柵化** 1080×2340(約桌機兩倍像素),比具名裝置嚴苛,因此該數字對真實行動硬體兩個方向都定不了案——數字與「為何無法定案」兩者皆已記錄,權威量測需實機,屬 ART-138。值得注意的是同一檔案下 `degraded` rung 達 60 fps,把成本定位在 Pixi stage 而非頁面其他部分。AC#7 的八小時以 `BENCH_SOAK_MINUTES` 參數化,已提交的是短跑,且明說它證明的是「harness 能偵測成長」而非「八小時乾淨」。`activeViewerCount`(可觀測性文件列 ART-136 為擁有者)刻意未建:收集它需要瀏覽器**回報**,那是 client write,而 `readOnlyClientBoundary` 禁止所有 client write primitive,故屬外部分析 sink,即 FR-Q007／ART-140 的範圍——可觀測性文件本身也是這樣寫的。見 `docs/dynamic-view-benchmarks.md` 與 `docs/benchmarks/` | ART-119, ART-120 |
...
| FR-Q007 動態分析事件（實現 §17） | New | ART-140, ART-47 | **Done（傳輸已於 ART-47 補齊）** | P1 | **塑造整個任務的限制是:「既有的合規收集機制」並不存在。** ART-47(隱私保護分析平台)仍是 To Do,本 repo 沒有任何 sink;而且客戶端**結構上也不可能自己發明一個**——`readOnlyClientBoundary` 禁止所有 client write primitive,`publicReadOnlyGuarantee.test.ts`(ART-128／FR-O009)斷言出貨 bundle 恰好只到達一個 Convex 表面且是 query。加一條回報用 mutation 不是把該保證延伸,而是在它上面開洞——`docs/dynamic-view-observability.md` 對它那兩個 `client_external` 指標得出的是同一個結論,並明寫收集器「多半是外部分析 sink 而非 Convex write path」。**故本任務交付除傳輸外的一切**:(1)契約——十七個事件一次宣告;(2)隱私保證——單一咽喉點的 allowlist 淨化器與對抗式測試;(3)發射點——實況表面上的真實觸發;(4)推導——§18.1 的兩個指標直接由事件流算出。預設 sink 丟棄,故上線本任務**完全不改變任何網路行為**,而這由 `analyticsSurface.test.ts` 以結構斷言(比照 `liveMapSurface.test.ts` 掃描 request／timer／identifier API)證明而非口頭保證。先做這一半是對的:一份**已證明乾淨且從正確位置發射**的事件契約是昂貴的那半,而把發射點和傳輸同時寫,意味著 payload 的錯誤會直接送到收集器而不是送到 no-op。**AC#2／#3 的核心是 allowlist 而非 denylist**:denylist 得列舉現在存在與未來新增的每一個私有欄位,而這些事件是從帶著 character projection、story overlay view model 與 replay frame 的元件裡發射的——私有資料在每個呼叫點都只隔一次屬性存取,漏掉的第一個欄位就是第一次外洩;allowlist 的失敗方向相反:沒想到的欄位被丟掉,事件只是資訊少一點。清單內**沒有任何觀看者識別碼**(無 viewerId／sessionId／IP／userAgent／自由文字),`characterId` 等是**世界**識別碼(每個 Episode 頁本來就印著),不是個人識別碼,而 §17 的點擊率不知道點了什麼就算不出來。另有三條規則各有理由:值必須是純量(巢狀物件**拒絕**而非遞迴走訪,因為那正是整個 view model 被意外附加到事件上的方式,而遞迴淨化器就得在每一層判斷什麼算私有);超過 64 字元的字串**丟棄而非截斷**(每個 allowlist 鍵放的都是識別碼或列舉成員,更長者依排除法就是散文,截斷仍會發布大半;測試釘住每個真實 Mistwood id 都放得下,故該上限不造成任何真實資料損失);**淨化發生在 `emitDynamicViewEvent` 內而非呼叫點**——若交給呼叫者就是紀律,放在唯一咽喉點就是結構,且 ART-47 之後安裝的 sink 無須知道它存在就會繼承它。**相機事件由「模式轉換」推導而非掛在各控制項上**,第二個理由是關鍵:「回到全鎮視角」同時關掉 follow **並且**清掉 focus,若各控制項各自發射,一次按壓會回報兩次互動,§18.1 的點擊率就會被灌水一個沒人會想到去查的量。**這個設計當場抓到一個缺陷**:第一版以 `focusId === TOWN_TARGET_ID` 判定全鎮視角,但那個 id 是**焦點目標清單**的項目,`CameraControls` 在它抵達 camera mode 之前就映射成 `focusId: null`,故沒有任何 mode 會等於它,`live_return_to_town` **永遠不會發射**——而且是靜默地,因為對不上任何前綴的 focus id 本來就會被忽略。DOM 測試抓到了。現改以 `focusId === null && !follow` 判定,正是該按鈕實際產生的狀態;連帶的後果明說:在未聚焦狀態下關掉 follow 會產生同樣的相機狀態,故也計為一次 return——那是誠實的讀法,因為 follow 開啟時相機停在**主要場景**,關掉它確實把觀看者移到了全鎮視角。同一個 DOM 測試還抓到第二個接線缺陷:開卡片的事件原本寫在 `LiveMapPage` 的 handler 而非 `LiveMapView` 的 wrapper,任何自備 `onOpenCharacterCard` 的呼叫者都會靜默地不發射任何東西。**`live_view_opened` 在任何資料抵達前就發射**,這是刻意的:它是 §18.1 點擊率的**分母**,必須對每一位抵達頁面的觀看者發射,包括投影從未載入與落在降級 rung 的那些;以載入成功為條件會剛好剔除點擊率最該關心的那些 session,而該指標會在頁面表現**越差時看起來越好**。算繪失敗同時產生 `live_map_failed` 與 `live_fallback_used`,兩者回答不同問題(「算繪器壞了嗎」與「多少 session 看到的不是地圖」),合併會讓後者對從來就沒有 WebGL 的瀏覽器無法回答。**`live_arc_opened` 從首頁發射**:實況地圖把主線 arc 算繪為**文字**(ART-125)而非連結,故實況表面上沒有 arc 可開,首頁第一畫面的 arc 連結(ART-129)是動態表面上唯一能開 arc 的地方——另一個選項是宣告該事件不可達,那對地圖為真、對產品為假。**AC#4 是已展示的性質而非承諾**:`summariseDynamicViewEvents` 由事件流算出點擊率與重播完成率,分母為零時回傳 `null` 而非 `0`(「沒人點」與「沒人來」是不同的發現,把後者報成前者正是一次上線被誤診為 UX 問題的方式),且跳過與完成分開計(跳過是觀看者的決定,不是失敗,混入完成率會讓兩個問題都答不了)。**驗證**:`npm run check` 全綠(156 套件、2422 通過、5 個既有 skip、build 成功);三次 fault injection (拆掉 allowlist→5 失敗;還原 town 判定缺陷→3 失敗;把開卡片的發射移回頁面→1 失敗)反證斷言非空過。**未涵蓋(已由 ART-47 補齊)**:傳輸本身、`activeViewerCount` 與 `rendererErrorRate`。ART-47(§15)蓋出的不是本欄當時預期的兩種形狀之一。**不是外部 sink**——第三方收集器看得到觀看者 IP,§15 的資料最小化規則禁止,而且會讓聚合結果落在本 repo 任何測試搆不到的地方;**也不是把 read-only 保證延伸**——而是**第四道 gate** `telemetry`,自己的模組、自己的 boundary、自己的上限,且政策**要求**它的 client root 與 viewer gate 的不相交,故沒有任何單一檔案能同時持有兩個 write 豁免。`viewerWriteBoundary.maxViewerMutations` 仍是 **2**,那就是「架構上分離」不只是名義的斷言。ART-140 這裡寫的設計論證全部成立且未更動——同一份 allowlist、同一個咽喉點、同樣拒絕走訪巢狀值——只是契約搬到 `convex/shared/analyticsContract.ts`,讓瀏覽器與**不受信任**的 ingest 跑同一個函式:兩份 allowlist 會漂移,而漂移是靜默的。瀏覽器閘門另外抓到兩個單元測試抓不到的缺陷:傳輸在送出中途到期的 flush 會**死鎖**(清掉自己的 timer handle 卻不重掛,`accept` 又因為 handle「看起來還在」而不補掛,之後排入的每一筆都永遠留在記憶體);以及 `AnalyticsTransport.tsx` 與 `analyticsTransport.ts` 在 macOS／Windows 上是同一個路徑,模組解析可能把邏輯檔解析成它自己的 importer,循環 import 讓 jest loader 無錯誤無輸出地卡死。見 `docs/dynamic-view-analytics.md` 與 `docs/product-analytics.md` | ART-118, ART-121 |
| FR-Q008 Dynamic MVP Release Gate（實現 §22） | New | ART-138 | To Do | P0 | — | ART-99, ART-141（ART-139 已完成）＋ 全部 P0 |

> **FR-Q001 的 4 項延後與其理由（ART-133）：** FR-Q001 列舉 11 項指標，其中 5 項於伺服器端實測（Runtime Projection 更新延遲、Snapshot 年齡、Canon／Runtime Location Mismatch、Missing Character Binding、Missing Location Binding），2 項為**結構性零值**（Public Mutation Attempt、Viewer-triggered LLM Call Count；由 ART-128 的公開函式面政策迭代證明，非計數器），其餘 4 項**明示標示為未量測並指派擁有者**，而非以永久 `0` 充數——儀表板上「永久 0」與「健康的量測值 0」無法區分，會誤導維運判讀（PRD FR-Q007 明文允許標示「未量測」而非估算）。(1) **Active Viewer 數量**與 (2) **Renderer Error Rate** 標為 `client_external`，因兩者皆需瀏覽器**寫入**回報，而 `architecture/module-boundaries.json` 的 `readOnlyClientBoundary` 與 ART-128 的安全套件正是禁止此路徑；登記擁有者原為 **ART-136**／**ART-137**，實際收集機制預期落在 FR-Q007／ART-140 的分析事件管線而非 Convex 寫入路徑。(3) **降級模式使用率**（FR-O010）與 (4) **Replay 播放次數與跳過率**（FR-O013）當時標為 `pending_feature`，於本節記錄當時被量測的功能尚不存在，故僅登記 registry 項目，不建欄位、不建計數器（沿用 `PUBLIC_MOTION_TYPES` 預留 `'replay'` 的既有慣例）。**現況更新（ART-178）：四項全部改列 `client_external`，owner 全部清空。** ART-127 的降級階梯與 ART-121 的 Visual Replay 皆已完成並實際運行，`LiveMapPage.tsx` 會送出 `live_fallback_used`／`live_replay_started`／`live_replay_skipped`／`live_replay_completed`；本段先前所寫「(4) 仍為 `pending_feature` 直到 ART-133 或其後續補上實際收集機制」**已由 ART-47 補上而不再成立**——ART-47 的 telemetry 管線在自己的 `telemetry` gate 後推導出 `active_live_viewers`、`renderer_error_rate`、`fallback_usage_rate` 與 `replay_play_rate`／`replay_skip_rate`／`replay_completion_rate`。四者在本 registry 仍回 `null`，因為伺服器確實看不到；差別在於 reason 現在同時說明「為何這裡看不到」與「數字在哪裡」，而不再宣稱一個已上線的功能不存在。另 AC#3 的「記錄」半邊：匿名拒絕**刻意不做持久化**，因 Convex mutation 為交易式、寫在拋出路徑上的資料列會被同一個拋出回滾，且未認證呼叫端可逐次寫入即構成儲存耗盡向量；改以 `anonymousDenialsDurable: null` ＋ 明文 reason 讓限制可見，並實測既有 `outcome: 'refused'` 稽核列數。詳見 `docs/dynamic-view-observability.md`。
...
| NFR2-002 效能 | New | **ART-136**（＝FR-Q005） | **Harness 已交付;AC#4 行動版實測未達標且無法在本主機定案** | P0 |
...
8. **分析事件不重建平台** — ART-140 只發出 §17 事件，平台由 **ART-47** 承接，並已交付：傳輸、儲存、§16.1 與 §18.1 的計算，以及 `activeViewerCount`／`rendererErrorRate` 兩個 `client_external` 指標的推導。
