...
| §5.2 | Product validation hypotheses (newcomer / arc / daily return / voting / tracking) | Non-launch measurement (instrumentation delivered) | ART-47 (Done) | Depends on live traffic. The measurement layer now exists: ART-47 ships the events, the store and the §16.1 computation. Structural enablers (comprehension suite ART-75, content eval ART-92) are Done; live validation remains a post-launch activity, not a launch gate |
...
| FR-G004 | Recap coverage & spoiler validation (high-importance coverage or explicit exclusion; major relationship change mentioned; turning point mentioned; spoiler violation detected) | P1 delivered early | ART-35 (Done, PR #115), ART-164 (made it an enforced publication precondition; both functions had zero callers) | `convex/recaps/coverageValidation.test.ts`, `convex/operations/postCommitLive.test.ts` |
...
| FR-I007 | Scoped relationship graph (current-arc core characters, one-hop relationships, last-7-day changes; date switch, type filter, summaries, change reasons; never renders all characters/relationships by default) | Delivered | ART-44 (Done) | Server-scoped read model `relationshipGraph:<worldId>:<worldDay>`, built from Canon (`convex/publicRead/relationshipGraphProjection.ts`) + `#graph/<worldId>` page. `convex/publicRead/relationshipGraphProjection.test.ts`, `src/components/public/relationshipGraphRoute.test.ts`, `e2e/dynamicView.spec.ts`. See `docs/scoped-relationship-graph.md`. (Full P1 graph accessibility remains ART-94) |
...
| RISK-002 | Boring world content → mitigations: tension readiness, warmup, Director pressure, repetition monitoring, heat score, long-absent character detection, environment injection | P0 delivered (heat score P1 deferred; repetition monitoring now delivered) | ART-7/8/19, ART-45, ART-88 (Done), ART-32 (To Do) | `tensionReadiness.test.ts`, `warmup.test.ts`, `director.test.ts`; viewer environment injection now lands via `environmentVoteInjection.test.ts`. Repetition monitoring is delivered by ART-88: `narrative.test.ts` and `textSimilarity.test.ts` for the measure, `longRunHarness.test.ts` for the fixed-seed ratio. Heat score (FR-F006/ART-32) remains P1 |
...
**None.** Every P0 clause (`§5.1` goals G1–G9, G12–G14; `§5.3` all tech/ops goals; UX-001..004, UX-006 + UX-005 principle; FR-A001..A004, FR-B001..B002, FR-C001..C005, FR-D001..D006, FR-E001..E003, FR-F001..F005, FR-G001..G003, FR-H001..H003, FR-I001..I006, FR-K001..K004, FR-K006, FR-L001..L003, FR-M001; the §12 pipeline & failure rules; §10.2 dev/test mode; NFR-003..006, NFR-008..009; §16.2 quality invariants; §19.1/19.2/19.4 + 7/30 of §19.3; RISK-001/003/004/006/007/008/009 and the P0 portions of RISK-002/005) is backed by one or more **Done** tasks. The only incomplete tasks referenced anywhere in this matrix are P1 or P2 (ART-32, ART-59, ART-71, ART-73, ART-76, ART-89, ART-90, ART-91, ART-94) — none of which own a P0 clause. ART-28 (FR-E005), ART-36 (FR-G005), ART-39 (FR-H004, §5.1 G10), ART-44 (FR-I007, NFR-002 graph clause), ART-46 (FR-J002), ART-47 (§15, §16.1), ART-52 (FR-M003 configuration clause), ART-58 (FR-M002 continuity half, §16.2 measurement) and ART-88 (FR-M002 narrative half, §16.2 重複場景比例 now met at 0 of 449) have since been delivered and are recorded above. One sub-clause remains genuinely open and is recorded as such rather than as closed: **FR-H004 AC#7's second clause** (explicit, authorized, lossless merging of anonymous and authenticated progress) is unsatisfiable while this deployment has no viewer authentication, and is owned by ART-71 (FR-J003), which is P1 and owns no P0 clause. Its first clause — no cross-identity read or write — is delivered and tested.
...
- **RISK-002 / RISK-005 partial:** the P1-deferred heat score (ART-32), token-rate controls (ART-59), and degradation workflow (ART-91) leave portions of these two risks' mitigation lists unimplemented. The novelty and repetition evaluators were on this list until ART-88 delivered them. Both are classified as "P0 mitigations delivered; refinement P1 deferred" because the P0 mitigations already bound the risk (tension readiness + Director pressure for RISK-002; scene merge + deterministic fake provider for RISK-005).
