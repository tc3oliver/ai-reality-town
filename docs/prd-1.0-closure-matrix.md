...
| `P1 delivered` — a P1 clause delivered anyway; not required for launch | 15 | No |
| `P2 delivered` — a P2 clause delivered anyway | 0 | No |
| `P1 deferred` — explicitly out of MVP, owned by a backlog task | 1 | No |
| `P2 deferred` — explicitly out of MVP, owned by a backlog task | 2 | No |
...
| FR-F006 | Arc heat score (traceable, not freely LLM-decided, operator-visible composition) | P1 deferred | ART-32 (To Do) | Does not block launch: arc priority for the homepage uses the deterministic portfolio/active-arc rules (FR-F004); heat score is a refinement that does not alter Canon or safety |
...
| FR-J003 | Authenticated follows & progress (follow characters/arcs, save progress, personalised return recap) | P2 deferred | ART-71 (To Do) | Explicit P2 in §17. No auth-required viewer surface in MVP |
