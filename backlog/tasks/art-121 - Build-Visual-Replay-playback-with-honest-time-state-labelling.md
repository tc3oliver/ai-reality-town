...
**Out of Scope:** Generating any new narrative content; scene visualization of the live active scene (FR-O003).

**Dependencies:** FR-O002 Canon-driven movement rendering.
...
**Security Impact:** Replay may only include already-published, safety-approved text; it must trigger no LLM call and no Canon write.

**Test Requirements:** Replay construction tests from fixed event fixtures; an integration test asserting replay adds no LLM trace and no accepted event; session auto-play-once tests; skip and manual-trigger tests.
