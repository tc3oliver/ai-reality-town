...
Added convex/safety/viewerInput.ts, the versioned FR-L003/NFR-005 classification layer that treats all viewer-submitted input as untrusted, plus convex/safety/viewerInput.test.ts and docs/viewer-input-safety.md.

classifyViewerInput rejects PROMPT_INJECTION, REAL_PERSON_REFERENCE, PERSONAL_DATA, UNSAFE_VIOLENCE_OR_SEXUAL_CONTENT, SYSTEM_COMMAND (shell, SQL, JSON schema escape, internal Convex API names, script/URI payloads, path traversal), and DIRECT_OUTCOME_CONTROL, plus structural INPUT_EMPTY, INPUT_TOO_LONG, and DISALLOWED_CHARACTERS. Rules run over several derived views (NFKC, Cyrillic/Greek homoglyph folding, invisible-character stripping, whitespace collapse, separator collapse, leet folding) and over payloads decoded from percent-encoding, HTML numeric entities, backslash-u escapes, and base64/base64url, so encoding tricks do not evade them. acceptViewerInput is fail-closed: rejected input throws and the consumer is provably never invoked, and no record or error message ever carries the submitted text, only stable codes and an FNV-1a fingerprint. Bounded viewer preferences remain accepted.

Scope: the classification layer only. Daily environmental voting (ART-45) is out of scope and remains To Do; this module lands first so no viewer surface can ship without a gate to call.

Verification evidence:
- npm run check -> green after merging origin/main: check:architecture, test:architecture, typecheck, lint all pass; Test Suites 72 passed/72; Tests 769 passed/769; build passes.
- npm test -- --runTestsByPath convex/safety/viewerInput.test.ts -> 73 passed/73, covering every listed attack category, every normalization and encoding variant, structural rejections, seven allowed bounded-preference cases, and the no-raw-text and consumer-never-invoked containment assertions.
- Architecture boundary check confirms convex/safety depends only on shared, so a viewer-input safety failure cannot modify Canon.
- PR https://github.com/tc3oliver/ai-reality-town/pull/117 (auto-merge enabled).
