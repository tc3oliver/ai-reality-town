...
export class OpenAICompatibleProvider implements LanguageModelProvider, SimulationProvider {
  ...
  private async request(url: string, body: Record<string, unknown>): Promise<{ body: unknown; retryCount: number; latencyMs: number }> {
    ...
    for (let attempt = 1; attempt <= this.config.maxAttempts; attempt += 1) {
      ...
      try {
        ...
        if (!transient || attempt === this.config.maxAttempts) break;
        await this.dependencies.delay(Math.min(100 * (2 ** (attempt - 1)), 1_000));
      } finally { clearTimeout(timeout); }
    }
    if (last instanceof SimulationProviderError) throw last;
    if (last instanceof DOMException && last.name === 'AbortError') throw new SimulationProviderError('transient', 'LLM_TIMEOUT', `provider exceeded ${this.config.timeoutMs}ms timeout`);
  ...
  async structuredChat(request: StructuredChatRequest): Promise<StructuredChatResult> {
    ...
    return { output, trace: { provider: 'openai-compatible', model: this.config.chatModel, ...tokens,
